  ...
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      page.drawText(title.toUpperCase(), {
        x: M, y: H - M - 16,
        size: 11, font: bold, color: rgb(0.2, 0.2, 0.2),
      });
      page.drawLine({
        start: { x: M, y: H - M - 22 },
        end: { x: W - M, y: H - M - 22 },
      ...
      page.drawRectangle({
        x: imgX - 2, y: imgY - 2,
        width: imgSize + 4, height: imgSize + 4,
        borderColor: rgb(0.85, 0.85, 0.85), borderWidth: 0.5,
      });

      const pageLabel = `Page ${i + 1} of ${images.length}`;
      page.drawText(pageLabel, {
        x: W / 2 - pageLabel.length * 3.5, y: M,
        size: 9, font: regular, color: rgb(0.6, 0.6, 0.6),
      });
      page.drawText('Color this page however you like!', {
        x: M, y: M,
        size: 9, font: regular, color: rgb(0.7, 0.7, 0.7),
      });
    }
    ...
    cover.drawText('MY COLORING BOOK', {
      x: W/2 - 118, y: H/2 + 80,
      size: 28, font: bold, color: rgb(0.15, 0.15, 0.15),
    });

    if (childName) {
      const forLine = safe('Created for ' + childName);
      cover.drawText(forLine, {
        x: W/2 - forLine.length * 5, y: H/2 + 28,
        size: 18, font: bold, color: rgb(0.97, 0.45, 0.09),
      });
    }

    const themeLine = safe(theme.toUpperCase() + ' ADVENTURE');
    cover.drawText(themeLine, {
      x: W/2 - themeLine.length * 5, y: H/2 - 10,
      size: 16, font: bold, color: rgb(0.4, 0.4, 0.4),
    });

    cover.drawText(images.length + ' pages to color', {
      x: W/2 - 55, y: H/2 - 60,
      size: 12, font: regular, color: rgb(0.6, 0.6, 0.6),
    });

    cover.drawText('colorbook.app', {
      x: W/2 - 35, y: M + 20,
      size: 9, font: regular, color: rgb(0.75, 0.75, 0.75),
    });
