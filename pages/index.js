...
async function buildPDF(b64Image) {
  ...
  // 8.5 x 11 inches at 72 DPI = 612 x 792 points
  const W = 612, H = 792;
  const page = doc.addPage([W, H]);
  const bytes = Uint8Array.from(atob(b64Image), c => c.charCodeAt(0));
  const png = await doc.embedPng(bytes);
  // Fill entire page edge to edge, no margins, no blank pages
...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Step 2: Generate ONLY page 1 for real
      setPS(0, 'generating');
      ...
      // Step 3: Instantly fill remaining pages with blurred preview (no API calls)
      ...
      try {
        ...
        setErrorMsg('Image generated! PDF failed: ' + pdfErr.message + ' — use Save PNG instead.');
      }
    ...
    <>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        body { font-family: 'Nunito', sans-serif; background: linear-gradient(150deg, #fff9f5 0%, #fef3e8 50%, #f0fdf4 100%); min-height: 100vh; }
      ...
      <div style={{maxWidth:900,margin:'0 auto',padding:'28px 20px 80px'}}>
        <div style={{textAlign:'center',marginBottom:32}}>
          ...
          <p style={{fontSize:15,color:'#78716c',fontWeight:600,maxWidth:460,margin:'0 auto'}}>
            Upload a photo · pick a theme · get a personalized coloring book
          </p>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 280px',gap:20,alignItems:'start'}}>
            ...
            <div style={{background:'#fff',borderRadius:18,border:'1px solid #f0ede8',padding:22,boxShadow:'0 2px 10px rgba(0,0,0,0.04)'}}>
              ...
              {photo ? (
                ...
                <div
                  ...
                  style={{border:`2px dashed ${dragging?'#f97316':'#d6d3d1'}`,borderRadius:14,padding:'32px 20px',textAlign:'center',cursor:'pointer',background:dragging?'#fff7ed':'#fafaf9',transition:'all 0.2s'}}
                >
              ...
              <div style={{background:'#fff',borderRadius:18,border:'1px solid #f0ede8',padding:22,boxShadow:'0 2px 10px rgba(0,0,0,0.04)'}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
                  ...
                  <div style={{display:'flex',gap:8}}>
                    {pageImages[0] && (
                      <button onClick={saveImage} style={{background:'#6366f1',color:'#fff',border:'none',borderRadius:10,padding:'8px 14px',fontFamily:'Nunito,sans-serif',fontWeight:800,fontSize:12,cursor:'pointer'}}>💾 Save PNG</button>
                    )}
                    {pdfBase64 && (
                      <button onClick={downloadPDF} style={{background:'#f97316',color:'#fff',border:'none',borderRadius:10,padding:'8px 14px',fontFamily:'Nunito,sans-serif',fontWeight:800,fontSize:12,cursor:'pointer'}}>⬇️ Download PDF</button>
                    )}
                  </div>
                </div>
                      ...
                      {pageImages[i] ? (
                        <>
                          <img
                            ...
                            alt={`Page ${i+1}`}
                            style={{width:'100%',height:'100%',objectFit:'cover',filter:pageStatuses[i]==='placeholder'?'blur(6px) brightness(1.05)':'none'}}
                          />
                          {pageStatuses[i]==='placeholder' && (
                            <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
                              <span style={{background:'rgba(249,115,22,0.9)',color:'#fff',fontSize:9,fontWeight:800,padding:'3px 7px',borderRadius:6}}>PREVIEW</span>
                            </div>
                          )}
                          {pageStatuses[i]==='done' && (
                            <div style={{position:'absolute',top:5,left:5,background:'#22c55e',color:'#fff',fontSize:9,fontWeight:700,padding:'2px 5px',borderRadius:4}}>✓</div>
                          )}
                        </>
                      ) : (
                        <div style={{width:'100%',height:'100%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:8}}>
                          <div style={{fontSize:18,animation:pageStatuses[i]==='generating'?'spin 1.2s linear infinite':'none'}}>
                            {pageStatuses[i]==='generating'?'⚙️':'🖼️'}
                          </div>
                ...
                {errorMsg && (
                  <div style={{marginTop:12,padding:'10px 14px',background:'#fef2f2',border:'1px solid #fecaca',borderRadius:10}}>
                    <p style={{fontWeight:700,fontSize:12,color:'#dc2626',margin:0}}>⚠️ {errorMsg}</p>
                  </div>
                )}

                {done && pdfBase64 && (
                  <div style={{marginTop:12,padding:'11px 14px',background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:12}}>
                    <p style={{fontWeight:800,fontSize:12,color:'#166534',margin:0}}>
                      ✅ Page 1 is your full personalized illustration on 8.5×11". Pages 2–{pageCount} are previews — hit Regenerate to create each unique page.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            <div style={{background:'#fff',borderRadius:18,border:'1px solid #f0ede8',padding:18,boxShadow:'0 2px 10px rgba(0,0,0,0.04)'}}>
              ...
              {[
                {icon:'📷',t:'AI reads your photo',d:'GPT-4o Vision captures hair, face, glasses and distinctive features.'},
                {icon:'🎨',t:'Personalized scene',d:'Your child is the hero with their actual appearance in every page.'},
                ...
                <div key={i} style={{display:'flex',gap:10,marginBottom:i<2?12:0}}>
                  ...
                  <div>
                    <p style={{fontWeight:800,fontSize:12,color:'#1c1917',margin:0}}>{it.t}</p>
                    <p style={{fontSize:11,color:'#78716c',margin:'2px 0 0',lineHeight:1.5}}>{it.d}</p>
                  </div>
                </div>
              ))}
            </div>
